...
public class StockCandlestickChartV2 extends Application {

    // 任务队列
    private final BlockingQueue<ChartTask> taskQueue = new LinkedBlockingQueue<>();
    private Stage mainStage;
    private boolean isAppRunning = false;
    private final Object lock = new Object(); // 用于同步启动
    ...
    private final int REQUIRED_LAYOUTS = 10; // 增加布局脉冲计数要求
    private int expectedCandleCount = 0; // 预期的蜡烛柱数量
    private long lastLayoutTime = 0; // 上次布局时间
    private final long MIN_RENDER_TIME = 50; // 最小渲染时间（毫秒）
    // 任务统计
    ...
    @Override
    public void start(Stage stageTeam) {
        if(mainStage == null){
            mainStage = stageTeam;
        }
    ...
    private synchronized void checkRenderCompletion() {
        if (chart == null || currentTask == null) {
            return;
        }
     ...
     */
    public void generateChart(List<StockData> stockData, String savePath,
            Consumer<File> successCallback,
            Consumer<Exception> errorCallback) {
    ...
    private void submitTask(ChartTask task) {
        try {
            // 如果应用尚未启动，则启动它
            if (!isAppRunning) {
                synchronized (lock) {
                    if (!isAppRunning) {
                        // 启动JavaFX应用
                        new Thread(() -> Application.launch(StockCandlestickChartV2.class)).start();
    ...
    // 创建改进版K线图
    private XYChart<String, Number> createCandlestickChart(List<StockData> stockDataList) {
        // 记录预期的蜡烛柱数量
    ...
    // 检查所有任务是否完成
    private boolean checkAllTasksCompleted() {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < allStockData.size(); i++) {
            ...
            new StockCandlestickChartV2().generateChart(
                    allStockData.get(i),
                    savePath,
